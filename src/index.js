...
const CANDLE_STRENGTH_MIN = 0.08; // relaxed from 0.15
const ATR_PCT_MAX = 6; // relaxed from 4 for more triggers

/* ============ NIFTY 100 SYMBOLS (cleaned) ============ */
const SYMBOLS = [
  ...
  // Removed ETERNAL.NS (invalid), TMCV.NS → TATAMOTORS.NS
];
...
function calculateConfidence({ ema9, ema21, rsi }) {
  ...
  if (rsi > 55 && rsi < 72) score += 30; // aligned with relaxed filter
...
async function runScanner() {
  ...
  const period1 = now - LOOKBACK_DAYS * 24 * 60 * 60;
  const period2 = now;
  ...
  for (const symbol of SYMBOLS) {
    ...
    // Proper 2-candle crossover window
    ...
    // Log for debug (uncomment if needed)
    // console.log(`${symbol} | EMA9:${ema9.toFixed(2)} > EMA21:${ema21.toFixed(2)} | RSI:${rsi.toFixed(2)} | Close:${entry.toFixed(2)} > EMA50:${ema50.toFixed(2)} | Candle:${candleStrength.toFixed(2)}% | ATR%:${(atr/entry*100).toFixed(2)}`);
    ...
    if (hr < 9 || (hr === 9 && min < 15) || hr > 15 || (hr === 15 && min > 30)) {
      // console.log(`${symbol} skipped: out of trading hours (${hr}:${min})`);
      continue;
    }
    ...
    // Add alert logic here (e.g., send to Telegram, email, etc.)
  }
